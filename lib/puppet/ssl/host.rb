  ...
  module SSL
    class Host
      ...
      def generate_certificate_request(options = {})
        ...
        csr_attributes = CertificateRequestAttributes.new(Puppet[:csr_attributes])
      ...
      def certificate
        unless @certificate
          ...
          sm = StateMachine.new(onetime: true)
      ...
      def validate_csr_with_key(csr, key)
        if key.content.public_key.to_s != csr.content.public_key.to_s
          error_info = { fingerprint: csr.fingerprint,
                         csr_public_key: csr.content.public_key.to_text,
                         agent_public_key: key.content.public_key.to_text,
                         cert_name: Puppet[:certname],
                         clean_params: clean_params,
                         puppet_params: puppet_params }
          raise Puppet::Error, _(<<-ERROR_STRING) % error_info
      ...
      def load_certificate_request_from_file
        ...
        if Puppet::FileSystem.exist?(request_path)
          CertificateRequest.from_s(Puppet::FileSystem.read(request_path))
        end
      end
      ...
      def download_csr_from_ca
        begin
          body = Puppet::Rest::Routes.get_certificate_request(
                        name, SSLContext.new(store: ssl_store))
          begin
            CertificateRequest.from_s(body)
      ...
      def submit_certificate_request(csr)
        Puppet::Rest::Routes.put_certificate_request(
                      csr.render, name, SSLContext.new(store: ssl_store))
      end
      ...
      def download_and_save_crl_bundle(store=nil)
        begin
          ...
          Puppet::Util.replace_file(crl_path, 0644) do |file|
            result = Puppet::Rest::Routes.get_crls(CA_NAME, SSLContext.new(store: store))
            file.write(result)
          end
        rescue Puppet::Rest::ResponseError => e
      ...
      def check_for_certificate_on_disk(cert_name)
        ...
        if Puppet::FileSystem.exist?(file_path)
          begin
            Certificate.from_s(Puppet::FileSystem.read(file_path))
      ...
      def download_certificate_from_ca(cert_name)
        begin
          cert = Puppet::Rest::Routes.get_certificate(
            cert_name,
            SSLContext.new(store: ssl_store)
          )
          begin
            Certificate.from_s(cert)
          rescue OpenSSL::X509::CertificateError
