...
agents.each do |host|
    ...
    on(host, puppet_resource("cron", "crontest", "user=#{tmpuser}",
      ...
      assert_match(/crontest\D+ensure:\s+removed/, stdout,
                   "Didn't remove crobtab entry for #{tmpuser} on #{host}")
    end

    step "verify that crontab -l contains what you expected"
    run_cron_on(host, :list, tmpuser) do
      assert_no_match(/\/bin\/true/, stdout,
                      "Error: Found entry for #{tmpuser} on #{host}")
    end
